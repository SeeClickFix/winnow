...
function handleExprList(node, options) {
  var values = node.value.map(function (valueNode) { return traverse(valueNode, options); }).join(',')
  return ("(" + values + ")")
}
...
function handleFunction(node, options) {
  var args = handleExprList(node.args, options)
...
function handleValue(node, options) {
  ...
  if (node.columnNode) {
    var field = _.find(options.esriFields, { name: node.columnNode.column })

    if (_.has(field, 'domain.codedValues')) {
      var actual = _.find(field.domain.codedValues, { code: value })
...
function createClause(options) {
  ...
  var whereTree = parser.parse('SELECT * WHERE ' + options.where).where
